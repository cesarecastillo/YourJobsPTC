...
namespace YourJob1
{
    public partial class _04_user_register_ESP : System.Web.UI.Page
    {
        ...
        protected void RegisterButton_Click(object sender, EventArgs e)
        {
            ...
            if (ValidateForm(firstName, lastName, gender, email, password, confirmPassword, age))
            {
                ...
                if (!EmailExists(email))
                {
                    ...
                    {
                        ShowAlert("Error de Registro", "Hubo un problema al registrar el usuario. Por favor, inténtelo de nuevo.", "error");
                    }
                }
                else
                {
                    ShowAlert("Usuario Ya Existe", "El correo electrónico ya está en uso. Por favor, use otro.", "error");
                }
            }
        }
        ...
        private bool ValidateForm(string firstName, string lastName, string gender, string email, string password, string confirmPassword, string age)
        {
            ...
            {
                ShowAlert("Error", "El nombre debe tener al menos 4 caracteres.", "error");
                return false;
            }
            if (lastName.Length < 4)
            {
                ShowAlert("Error", "El apellido debe tener al menos 4 caracteres.", "error");
                return false;
            }
            if (string.IsNullOrEmpty(gender))
            {
                ShowAlert("Error", "Por favor, seleccione un género.", "error");
                return false;
            }
            if (email.Length < 10 || !IsValidEmail(email))
            {
                ShowAlert("Error", "Por favor, ingrese una dirección de correo electrónico válida.", "error");
                return false;
            }
            ...
            {
                ShowAlert("Error", "La edad debe ser un número entre 18 y 65.", "error");
                return false;
            }
